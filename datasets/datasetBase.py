...
class DatasetWrapper(torch.utils.data.Dataset):   
    def __init__(self, model, dataset, img_save_folder, label_save_folder, img_transform=None, bbox_transform=None):
        self.model = model
        self.dataset_raw = dataset
        self.img_save_folder = img_save_folder
        self.label_save_folder = label_save_folder
        self.img_transform = img_transform
        self.bbox_transform = bbox_transform
        self.dataset_transform_for_model()

    def dataset_transform_for_model(self):
        for index in range(len(self.dataset_raw)):
            img, annotations = self.dataset_raw[index]
            if not annotations:
                with open("./log.txt", "a+") as file:
                    file.write("missing annotation\n")
                continue
            img_id = annotations[0]['image_id']
            file_img_label_path = f"{self.label_save_folder}/00100{img_id}.txt"
            image_save_path = os.path.join(self.img_save_folder, f"00100{img_id}.jpg")
            id_check = f"00100{img_id}"
            if self.img_transform is not None:
                original_width, original_height = img.size
                img = self.img_transform(img)
                file_img_label_path = f"{self.label_save_folder}/{img_id}.txt"
                image_save_path = os.path.join(self.img_save_folder, f"{img_id}.jpg")
                id_check = f"{img_id}"
                # img = ToPILImage()(img)
                # img.save(image_save_path)           
            if self.bbox_transform is not None:
                annotations = self.bbox_transform(original_width, original_height, annotations)
            file_img_label = open(file_img_label_path, "a+")
            print(id_check)
            if self.model == 'yolo5':
                boxes = []
                for ann in annotations:
                    # In COCO, categories start from 1, while in YOLO5 they start from 0
                    new_category = ann['category_id'] - 1 
                    old_bbox = ann['bbox']
                    x = old_bbox[0]
                    y = old_bbox[1]
                    w = old_bbox[2]
                    ...
                    x1 = (x_centre - w / 2) * img_width
                    y1 = (y_centre - h / 2) * img_height
                    x2 = (x_centre + w / 2) * img_width
                    y2 = (y_centre + h / 2) * img_height
                    ...
                    file_img_label.write(f"{new_category} {x_centre} {y_centre} {w} {h}\n")
                ...
                img_with_boxes = draw_bounding_boxes(img, boxes_tensor, colors=box_color, width=2)
...
class PoisonedDatasetWrapper(torch.utils.data.Dataset):
    ...
    def transform_save_bd(self):
        ...
        for index, poisoned in enumerate(self.poisoning_indices):
            if poisoned == 1:
                ...
                # and on the disk
                self.bd_dataset[bd_index] = (bd_img, bd_annotations)
                #image_save_path = os.path.join(self.images_save_folder, f"00100{img_id}.jpg")
    ...
    def __getitem__(self, index):
        img, annotations = self.bd_dataset[index]
        return (img, annotations)
